...
def main(stdscr):
    ...
    player_win = curses.newwin(2, 30, 14, 10)
    dealer_win = curses.newwin(2, 30, 1, 10)
    ...
    while GameOver == False:
        if game.playerStatus == 'P':
            ...
            dealer_win.addstr( f'Dealer Hand: {game.dealerHand.ShowHand()} \nDealer Count: {game.dealerHand.GetHandValue()} ')
            
            stdscr.addstr( 7, 10, 'Press H to Hit')
            stdscr.addstr( 8, 9, 'Press S to Stand')
            player_win.addstr( f'Player Cards:{game.playerHand.ShowHand()} \nPlayer Count: {game.playerHand.GetHandValue()}')
